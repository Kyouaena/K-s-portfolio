        ...
        {projects.items.map((project, index) => {
          const card = (
            <>
              ...
              <h3 className="mt-2 text-xl font-bold text-ink md:text-2xl group-hover:text-accent-glow">
                {project.name}
              </h3>
          ...
          if (project.demoSlug) {
            return (
              <Link
                key={project.id}
                href={`/demo/${project.demoSlug}`}
                className="group block rounded-2xl border border-surface-border bg-gradient-to-br from-surface-raised to-surface p-6 transition hover:border-accent/50 md:p-8"
              >
                {card}
              </Link>
            );
